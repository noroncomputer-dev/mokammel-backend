...
export default function EditProductPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const flavorsAsString = formData.flavors
        .filter((f) => f.name.trim())
        .map((f) => f.name);

      const ingredientsFiltered = formData.ingredients.filter((i) => i.trim());
  ...
  if (initialLoading) {
    return (
      ...
      >
        ...
        <span className="text-muted-foreground">در حال بارگذاری محصول...</span>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6" dir="rtl">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="badge-gold text-xs">شناسه: {productId.slice(-8)}</div>
      </div>
      ...
      {success && (
        <div className="flex items-center gap-2 bg-emerald-500/10 border border-emerald-500/30 text-emerald-600 dark:text-emerald-400 px-4 py-3 rounded-xl text-sm backdrop-blur-sm">
          ...
          محصول با موفقیت ویرایش شد! در حال انتقال...
        </div>
      )}
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div>
                  <label className={labelClass}>
                    <Beef className="h-3 w-3 inline ml-1" />
                    پروتئین (گرم)
                  </label>
                  <input
                    type="number"
                ...
                <div>
                  <label className={labelClass}>
                    <Droplet className="h-3 w-3 inline ml-1" />
                    چربی (گرم)
                  </label>
                  <input
                    type="number"
